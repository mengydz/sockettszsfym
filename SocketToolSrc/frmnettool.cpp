...
void frmNetTool::initForm()
{
    ...
    setWindowTitle("Socket调试助手 V1.0");
...
void frmNetTool::appendTcpClient(quint8 type, QString msg)
{
	...
	if (type == 0) {
        ...
        str = "系统信息 :";
        ui->txtDataTcpServer->setTextColor(QColor("blue"));
    }

    ui->txtDataTcpClient->append(QString("[%1] %2 %3").arg(TIMEMS).arg(str).arg(msg));
	countTcpClient++;
}
...
void frmNetTool::appendTcpServer(quint8 type, QString msg)
{
    ...
    ui->txtDataTcpServer->append(QString("[%1] %2 %3").arg(TIMEMS).arg(str).arg(msg));
	countTcpServer++;
}
...
void frmNetTool::on_btnTcpListen_clicked()
{
	if (ui->btnTcpListen->text() == "监听") {
		...
		if (ok) {
            ...
            appendTcpServer(2, "监听成功");
		} else {
            appendTcpServer(2, "监听失败,请检查端口是否被占用");
		}
	} else {
		ui->listTcpClient->clear();
		tcpServer->closeAll();
...
void frmNetTool::on_btnTcpConnect_clicked()
{
	if (ui->btnTcpConnect->text() == "连接") {
		...
		if (tcpClient->waitForConnected(1000)) {
            ...
            appendTcpClient(2, "连接服务器成功");
		} else {
            appendTcpClient(2, "连接服务器失败");
		}
	} else {
		tcpClient->disconnectFromHost();
...
void frmNetTool::on_btnUdpListen_clicked()
{
	if (ui->btnUdpListen->text() == "监听") {
		...
		if (ok) {
            ...
            appendUdpServer(2, "监听成功");
		} else {
            appendUdpServer(2, "监听失败,请检查端口是否被占用");
		}
	} else {
		udpServer->abort();
		ui->btnUdpListen->setText("监听");
		ui->btnSendUdpServer->setEnabled(false);
        appendUdpServer(2, "停止监听成功");
	}
}
